...
export const api = {
  ...
  testAllCandidates: (provider?: string) => request<{ totalChecked: number; results: Array<{ id: string; status: string; speedMs?: number | null; error?: string | null }> }>("/admin/candidates/test-all", { method: "POST", body: JSON.stringify({ provider }) }),
