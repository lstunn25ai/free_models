...
export const api = {
  ...
  testAllCandidates: (input: { provider: string; scope: "AVAILABLE" | "ALL"; confirmPaidUnknown?: boolean }) => request<{ provider: string; scope: "AVAILABLE" | "ALL"; totalChecked: number; results: Array<{ id: string; status: string; speedMs?: number | null; error?: string | null }> }>("/admin/candidates/test-all", { method: "POST", body: JSON.stringify(input) }),
